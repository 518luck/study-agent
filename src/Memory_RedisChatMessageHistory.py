...
# 关掉本文件的「缺失导入」检查：langchain-redis 属可选依赖，未安装时下面的 import 解析不到，
# 运行时会由 except 分支回退到 langchain-community，属预期情况。
# （不用行尾 # pyright: ignore：那样会与 ruff 的 import 排序规则打架）
...
def get_session_history(session_id: str) -> BaseChatMessageHistory:
    ...
    if USE_LANGCHAIN_REDIS:
        # pyright 会把上面的名字解析成 community 版（它的参数叫 url），但此分支用的是 langchain-redis 的参数名
        return RedisChatMessageHistory(
            session_id=session_id,
            redis_url=REDIS_URL,  # pyright: ignore[reportCallIssue]
        )
    return RedisChatMessageHistory(
        session_id=session_id,
        url=REDIS_URL,
