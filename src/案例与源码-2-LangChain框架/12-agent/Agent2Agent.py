...
def _raw_func(tool_obj: BaseTool) -> Callable[..., Any]:
    """取出 @tool 装饰前的原始函数（StructuredTool.func），兜底时直接调用它。

    两点说明：
    - @tool 装饰后运行时是 StructuredTool，但类型标注上只推断为 BaseTool，
      所以这里用 cast 明确类型，才能访问 .func（pyright 不会报错）。
    - .func 的标注是「可调用或 None」（用 args_schema 构造的工具可能没有原始函数），
      这里明确检查并抛错，调用处就不用各自判断了。
    """
    func = cast(StructuredTool, tool_obj).func
    if func is None:
        raise ValueError(f"工具 {tool_obj.name} 没有可用的原始函数")
    return func
...
def create_travel_coordinator_agent(llm, ctrip_chain, meituan_chain, didi_chain):
    ...
    def a2a_schedule(input_dict):
        ...
        # .func 是装饰前的原始函数，兜底时直接调用，不经过模型（参数按位置传）
        ctrip_func = _raw_func(ctrip_book_flight)
        meituan_func = _raw_func(meituan_book_hotel)
        didi_func = _raw_func(didi_book_taxi)
