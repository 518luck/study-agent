...
- 若先把状态字典赋给变量再传入 invoke()，变量需显式标注 `initial_state: BasicState = {...}`，否则会被推断为普通 dict 而类型报错。
...
# 标注 BasicState（等价于 TS 的 const initial_state: BasicState = {...}）：不标注时变量被推断为普通 dict，传给 invoke 会类型报错
